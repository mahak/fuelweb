...
In a nutshell, the Fuel deployment orchestration engine, Astute, manages
OS provisioning via Cobbler, and uses an MCollective plugin to
distribute a Facter facts file that defines node's role and other
deployment variables for Puppet. You can find a detailed breakdown of
how this works in the :doc:`Sequence Diagrams </develop/sequence>`.
...
Communications between these components are illustrated on the Logical
